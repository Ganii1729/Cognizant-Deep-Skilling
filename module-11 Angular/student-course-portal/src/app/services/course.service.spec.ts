...
describe('CourseService', () => {
  ...
  it('should fetch courses via HTTP GET request', () => {
  ...
  it('should create a course via HTTP POST request', () => {
    const newCourse = { name: 'Algorithms', code: 'CS303', credits: 3, gradeStatus: 'pending' as const };

    service.createCourse(newCourse).subscribe(created => {
      expect(created.id).toBe(3);
      expect(created.name).toBe('Algorithms');
    });
